...
@Component({
  ...
  template: `<div class="dashboard-container">
    <header class="dashboard-header"><div class="header-content"><h1>My Students</h1><div class="user-info"><span>Welcome, {{ user?.name }}!</span><button (click)="logout()" class="logout-btn">Logout</button></div></div></header>
    <nav class="dashboard-nav"><div class="nav-content"><a routerLink="/dashboard/instructor" class="nav-item">📊 Overview</a><a routerLink="/dashboard/instructor/courses" class="nav-item">📚 My Courses</a><a routerLink="/dashboard/instructor/upload" class="nav-item">⬆️ Upload Content</a><a routerLink="/dashboard/instructor/students" routerLinkActive="active" class="nav-item">👥 Students</a><a routerLink="/dashboard/instructor/profile" class="nav-item">👤 Profile</a></div></nav>
    <main class="dashboard-main"><h2>Student Management - Coming Soon</h2><p>Student progress tracking will be available here.</p></main></div>`,

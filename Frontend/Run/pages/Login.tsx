...
const LoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#6B73FF] to-[#A855F7] p-4 flex flex-col items-center justify-center">
      <div className="w-full max-w-4xl">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden">
          {/* Left Side */}
          <div className="w-full md:w-1/2 p-8 sm:p-12 bg-white/50 text-slate-800 flex flex-col justify-center">
            <div className="mb-8">
              <h1 className="text-4xl font-bold text-slate-900">SkillForge</h1>
              <p className="text-2xl text-slate-700 mt-2">Welcome Back</p>
            </div>
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    id="email"
                    type="email"
                    className="pl-10 bg-slate-100 border-slate-300 focus:bg-white"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 pr-10 bg-slate-100 border-slate-300 focus:bg-white"
              ...
              <Button type="submit" className="w-full !mt-8 bg-gradient-to-r from-purple-600 to-indigo-600 text-lg py-3 h-auto" disabled={isLoading}>
          ...
          {/* Right Side */}
          <div className="w-full md:w-1/2 p-8 sm:p-12 bg-slate-50 flex flex-col justify-center">
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-bold text-slate-800">SkillForge Platform</h2>
                <p className="text-slate-600 mt-2">Advanced learning management system with role-based access for enhanced educational experiences. <span className="font-semibold">Click a role to sign in with demo credentials.</span></p>
              </div>
              <div className="space-y-2">
                <RoleInfoCard
                  icon={<GraduationCapIcon className="w-6 h-6 text-indigo-500" />}
                  title="Students"
                  description="Access courses, take assessments, and track your learning progress."
                  onClick={() => handleDemoLogin('student')}
                />
                <RoleInfoCard
                  icon={<PresentationIcon className="w-6 h-6 text-green-500" />}
                  title="Instructors"
                  description="Create and manage courses, generate AI-powered content, and monitor student progress."
                  onClick={() => handleDemoLogin('instructor')}
                />
                <RoleInfoCard
                  icon={<UserCogIcon className="w-6 h-6 text-red-500" />}
      ...
      {/* Feature Highlights */}
      <div className="w-full max-w-4xl mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 text-white">
        <FeatureCard
        ...
        />
        <FeatureCard
          icon={<AreaChartIcon className="w-8 h-8"/>}
...
const RoleInfoCard: React.FC<{ icon: React.ReactNode; title: string; description: string; onClick: () => void; }> = ({ icon, title, description, onClick }) => (
    <button
        type="button"
        onClick={onClick}
        className="flex w-full items-start gap-4 p-4 rounded-lg hover:bg-slate-200/50 transition-colors text-left focus:outline-none focus:ring-2 focus:ring-indigo-500"
    >
        <div className="bg-white rounded-lg p-2 shadow-md shrink-0">{icon}</div>
        <div>
...
const FeatureCard: React.FC<{ icon: React.ReactNode; title: string; description: string; }> = ({ icon, title, description }) => (
    <div className="bg-white/10 backdrop-blur-md p-6 rounded-xl flex flex-col items-center text-center">
...
const AreaChartIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
